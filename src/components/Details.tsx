              ...
              <Col xs={6} className="d-flex flex-column">
                <div className="mb-4" style={{ position: 'relative', flex: 1, overflow: 'hidden', whiteSpace: 'nowrap'}} ref={container}>
                  {data.photos.edges.map((img, index) => (
                    <GatsbyImage loading="eager" style={{ position: 'absolute', left: 0, top: 0, width: '100%', height: '100%', zIndex: index === currentIndex ? 1 : 0, willChange: 'zIndex', transform: 'translateZ(0)' }} objectFit={'cover'} image={getImage(img.node)} key={index} alt={`Detailaufnahme bei der Arbeit Bildsequenz Index ${index}`} />
                  ))}
                </div>
                <div style={{ flex: 1}}>
